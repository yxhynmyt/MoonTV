...
def count_journal_files(dev_dir: Path, active_num: int) -> str:
    ...
    for f in files:
        ...
        status = "活跃" if filename == active_file else "已归档"
...
def create_new_journal_file(
    ...
    content = f"""# 工作日志 - {developer} (第 {num} 部分)

> 承接自 `{FILE_JOURNAL_PREFIX}{prev_num}.md`（约 {max_lines} 行后归档）
> 开始日期: {today}
...
|------|------|"""
        ...
        commit_table = "（无提交，本次为规划会话）"

    package_line = f"\n**模块**: {package}" if package else ""
...
def update_index(
    ...
    for line in lines:
        if "@@@auto:current-status" in line:
            ...
            new_lines.append(f"- **当前活跃文件**: `{active_file}`")
            new_lines.append(f"- **总会话数**: {new_session}")
            new_lines.append(f"- **最后活跃时间**: {today}")
            continue
        ...
        if in_session_history:
            ...
            if re.match(r"^\|\s*#\s*\|\s*Date\s*\|\s*Title\s*\|\s*Commits\s*\|\s*$", line):
                new_lines.append("| # | 日期 | 标题 | 提交 | 分支 |")
                continue
            if re.match(r"^\|\s*#\s*\|\s*日期\s*\|\s*标题\s*\|\s*提交\s*\|\s*分支\s*\|\s*$", line):
                new_lines.append("| # | 日期 | 标题 | 提交 | 分支 |")
                continue
...
def main() -> int:
    ...
    parser.add_argument("--summary", default="（补充摘要）", help="Brief summary")
